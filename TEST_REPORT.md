...
- ✅ Cursor control: `clearScreen`, `hideCursor`, `showCursor`, `moveCursor`, `clearLine`
...
- ✅ `parseArgs` - simple options, with value, with equals, short options, arguments, defaults
...
- ✅ Help output includes subcommand aliases (e.g. `generate (g)`, `migrate (m)`) - verified via subprocess
...
**Test scenarios**:
- ✅ `table`, `keyValueTable`, `progressBar`, `progressBarLive` / `progressBarLiveFinish`
- ✅ Edge cases: empty data, single-row, different types, progress bar 0%/50%/100%
...
**Test scenarios**:
- ✅ Function existence: `prompt`, `confirm`, `select`, `multiSelect`, `input`, `inputEmail`, `inputNumber`, `inputPassword`, `inputUsername`, `pause`, `interactiveMenu`, `interactiveMultiMenu`, `interactiveMenuSearch`
- ✅ **Prompt subprocess tests** (via `createCommand` + `execPath` from @dreamer/runtime-adapter):
...
- ⏭️ `shouldUseColor` skip Linux Docker detection - **Skipped** (platform() !== "windows")
...
2. **Prompt subprocess tests**: select, multiSelect, confirm, input, inputEmail, inputNumber, inputUsername, readLine verified via subprocess
...
1. **Bun compatibility**: Updated @dreamer/runtime-adapter to 1.0.2 for Bun stdin getWriter support
...
1. **Interactive features**: Prompt interactive features (interactiveMenu/Multi/Search) mainly validate interface existence

2. **Skipped test**: `Windows: shouldUseColor 应跳过 Linux Docker 检测` - only runs on Windows platform
...
**Test framework**: @dreamer/test@^1.0.0 (Deno and Bun compatible)
**Test environment**: `deno test -A tests` | `bun test tests`
**Total tests**: 141 (140 passed, 1 skipped)
**Pass rate**: 100% ✅
